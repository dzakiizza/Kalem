...
class OnBoarding: UIViewController, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        for index in 0..<desc.count {
            ...
            let txt = UILabel(frame:CGRect(x: labelTitle.frame.minX + 32, y: self.view.center.y - 120, width: 300,height: 220))
            txt.textAlignment = .center
            txt.font = UIFont(name: "SF Pro-Light", size: 1.0)
            txt.font = UIFont.systemFont(ofSize: 24.0)
            txt.numberOfLines = 0
            txt.textColor = .white
            txt.text = desc[index]
            ...
            if index == 1 {
                ...
                let btn = UIButton(frame: CGRect(x: txt.frame.minX - 15, y: self.view.center.y + 170, width: 334, height: 47))
